...
public class MemgaipDao {
	...
	//num과 pass받아서 비밀번호 같으면 true, 틀리면 false
	...
	public void updateMemgaip(MemgaipDto dto) {
		...
		String sql ="update memgaip set m_hp=?, m_name=?, m_photo=? where m_num=? ";
		
		try {
			pstmt=conn.prepareStatement(sql);
			
			pstmt.setString(1, dto.getM_hp());
	...
	//해당 번호의 내용 보기
	public MemgaipDto getContent(String m_num) {
		MemgaipDto dto = new MemgaipDto();
